#!/usr/bin/env python3
"""
Package the Whitebox AI Agent plugin for upload to the official QGIS plugin repository.

This script creates a zip file with the following characteristics:
- Root folder named 'whitebox_agent'
...
def package_plugin(
    ...
    target_name: str = "whitebox_agent",
    ...
    Args:
        source_dir: Path to the whitebox_agent directory
...
def main():
    parser = argparse.ArgumentParser(
        description="Package Whitebox AI Agent plugin for repository upload",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Source directory (default: whitebox_agent in the same folder)",
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        default="whitebox_agent",
        help="Target plugin name for the zip root folder",
    )
    parser.add_argument(
        "--no-version",
        action="store_true",
    ...
    source_dir = args.source if args.source else script_dir / "whitebox_agent"
